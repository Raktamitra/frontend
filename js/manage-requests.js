...
//         console.error("Search or filter input not found in the DOM.");
//         return;
//     }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('http://localhost:5000/api/admin/requests')
        ...
        .then(data => {
            ...
            if (data.success) {
                allRequests = data.requests;
                renderBloodRequests(allRequests);
            } else {
                console.error('Error loading requests:', data.message || data);
            }
        })
    ...
    const searchBox = document.getElementById("request-search");

    if (searchBtn) searchBtn.addEventListener("click", applySearchAndFilter);
    if (filterBtn) filterBtn.addEventListener("click", applySearchAndFilter);
    if (searchBox) {
    ...
    // Optional: if you have initAdmin() or initManageRequests() functions in other files
    if (typeof initAdmin === 'function') initAdmin();
    if (typeof initManageRequests === 'function') initManageRequests();
});
